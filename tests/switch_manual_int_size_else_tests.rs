...
mod switch_manual_int_size_else;
use switch_manual_int_size_else::*;
...
fn basic_parse() {
    ...
    let mut test = SwitchManualIntSizeElse::default();
    {
    ...
    if let SwitchManualIntSizeElse_Chunk_Body::SwitchManualIntSizeElse_Chunk_ChunkMeta(s) =  test.chunks[0].body.as_ref().unwrap() {
    ...
    if let SwitchManualIntSizeElse_Chunk_Body::SwitchManualIntSizeElse_Chunk_ChunkDir(s) =  test.chunks[1].body.as_ref().unwrap() {
    ...
    if let SwitchManualIntSizeElse_Chunk_Body::SwitchManualIntSizeElse_Chunk_Dummy(s) =  test.chunks[2].body.as_ref().unwrap() {
    ...
    if let SwitchManualIntSizeElse_Chunk_Body::SwitchManualIntSizeElse_Chunk_Dummy(s) =  test.chunks[3].body.as_ref().unwrap() {
        let raw : Vec<u8> = vec![];
        assert_eq!(raw, *s.rest());
    } else {
        panic!("expected enum SwitchManualIntSizeElse_Chunk_Dummy");
